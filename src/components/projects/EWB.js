import React, { Component } from 'react';
import '../../App.css';

export default class EWB extends Component {
  render() {
    return (
      <div className="animated fadeInUpBig">
        <span><h2 className="project-title">Engineers Without Borders</h2><hr /></span>
        <p>> Engineers Without Borders is an organization that aims to fight systemic change and use
            design thinking to create solutions for problems like poverty, education, and sustainability.
            The focus of the chapter during my term as part of the executive team was aboriginal issues,
            as well as engineering advocacy. I helped start a podcast that interviewed students about campus
            issues, their experiences in engineering, as well as an open forum for the issues that EWB tackled.
            I also helped run outreach events at elementary schools and worked on getting the university Fair
            Trade certified.</p>
        <span className="project-button">{this.props.button}</span>
      </div>
    );
  }
}
